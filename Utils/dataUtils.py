...
class QueryAudioDataset(Dataset):
    def __init__(self, audio_feature, data_df, query_col, vocabulary):
        ...
        self.query_col = (
            query_col  # ? Column in _captions.json pd that relates to the NER tags
        )
    ...
    def __getitem__(self, index):
        ...
        query = torch.as_tensor(
            [self.vocabulary(token) for token in item[self.query_col]]
        )
...
def load_data(config):
    ...
    for word in word_vectors:
        if len(vocabulary) == 0:
            ...
            vocabulary.add_word(
                "<pad>", np.zeros_like(word_vectors[word])
            )  # ? Create pad vector
