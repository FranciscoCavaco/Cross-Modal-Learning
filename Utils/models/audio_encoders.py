...
#? Depending on the conf.yaml, but default output is (batch, 300)
class CNNModule(nn.Module):
    ...
    def forward(self, x):
        ...
        :return: tensor, (batch_size, time_steps / 4, 128 * Mel_bands / 64). ? shouldn't be / 64
        """
        x = x.unsqueeze(1)
        x = self.features(x)
...
class CRNNEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.mean(x, dim=1, keepdim=False) #? average all the tensors
